function aggiornaColoreValore(indirizzo) {
    ...
    // A (default)
    let byteA = null;
    if (typeof ultimoParametro !== "undefined" && ultimoParametro) {
        byteA = Number(ultimoParametro.VALORE_DEFAULT ?? ultimoParametro.VALORE)
            .toString(16).toUpperCase().padStart(2, "0");
    }

    // B (memoria B)
    let byteB = null;
    if (typeof memB !== "undefined" && memB && memB[indirizzo] != null) {
        byteB = memB[indirizzo].toString(16).toUpperCase().padStart(2, "0");
    }

    // C (memoria C)
    let byteC = null;
    if (typeof memC !== "undefined" && memC && memC[indirizzo] != null) {
